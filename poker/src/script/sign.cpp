                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
	...
	case TX_IF_ELSE:
	{
		CKeyID elsekeyID;
		keyID = CKeyID(uint160(vSolutions[0]));
		elsekeyID = CKeyID(uint160(vSolutions[1]));
		if (!Sign1(keyID, creator, scriptPubKey, ret, sigversion))
		{
			ret.clear();
			if (!Sign1(elsekeyID, creator, scriptPubKey, ret, sigversion))
				return false;
			else
			{
				CPubKey vch;
				creator.KeyStore().GetPubKey(elsekeyID, vch);
				ret.push_back(ToByteVector(vch));
				return true;
			}
			return false;
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
	...
	else if (solved && whichType == TX_IF_ELSE)
    {
	...
	else if (solved && whichType == TX_POKER)
    {
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata,  std::vector<unsigned char> handcards,  std::vector<unsigned char>* cards)
{
    CScript script = fromPubKey;
    std::vector<valtype> result;
    txnouttype whichType;
    ...
    // Test solution
    return solved && VerifyScript(sigdata.scriptSig, fromPubKey, &sigdata.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker(), nullptr, cards);
}