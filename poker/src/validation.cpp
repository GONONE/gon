...
static bool AcceptToMemoryPoolWorker(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache,std::string ip)
{
...

    {
        ...
        pool.addUnchecked(hash, entry, setAncestors, validForFeeEstimation, true,ip);
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState &state, const CTransactionRef &tx, bool fLimitFree,
                        bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced,
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee,std::string ip)
{
    std::vector<COutPoint> coins_to_uncache;
    bool res = AcceptToMemoryPoolWorker(chainparams, pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime, plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache,ip);
    if (!res) {
                        ...
                        bool fOverrideMempoolLimit, const CAmount nAbsurdFee,std::string ip)
{
    ...
    return AcceptToMemoryPoolWithTime(chainparams, pool, state, tx, fLimitFree, pfMissingInputs, GetTime(), plTxnReplaced, fOverrideMempoolLimit, nAbsurdFee,ip);
}
...
bool CScriptCheck::operator()() {
	...
	int size = ptxTo->vout.size();
	if(size > 1){
		CScript spk = ptxTo->vout[size - 1].scriptPubKey;
		if(spk[0] == OP_RETURN && spk[1] == OP_POKER){
			for(size_t i = 3; i< spk.size(); i++){
				cards.push_back(spk[i]);
			}
		}
	}
	
    return VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, cacheStore, *txdata), &error, &cards);
}
