...
static bool GetUTXOStats(CCoinsView *view, CCoinsStats &stats)
{
	...
	stats.hashBlock = pcursor->GetBestBlock();
	{
		...
		std::cout << "nHeight   :  " << stats.nHeight << std::endl;
	}
	ss << stats.hashBlock;
	uint256 prevkey;
...
UniValue getaddressutxo(const JSONRPCRequest& request)
{
	if (request.fHelp || request.params.size() != 1)
		throw std::runtime_error("getaddressutxo (address) \n");

	FlushStateToDisk();
	std::unique_ptr<CCoinsViewCursor> pcursor(pcoinsdbview->Cursor());

	if (!pcoinsdbview)
		std::cout << "pcoinsdbview empty" << std::endl;

	CHashWriter ss(SER_GETHASH, PROTOCOL_VERSION);
	int bestHeight;
	{
		LOCK(cs_main);
		bestHeight = mapBlockIndex.find(pcursor->GetBestBlock())->second->nHeight;
	}
	...
	while (pcursor->Valid())
	{
		...
		if (pcursor->GetKey(key) && pcursor->GetValue(coin))
		{
			if(coin.fCoinBase && coin.nHeight + 3 > bestHeight)
			{
				pcursor->Next();
				continue;
			}
			...
			if (fValidAddress)
			{
				...
				if(addr == str)
				{
					...
					entry.push_back(Pair("amount", coin.out.nValue));
	...
	res.push_back(Pair("utxo", results));
	res.push_back(Pair("blockheight", bestHeight));
	return res;
}
...
{ //  category              name                      actor (function)         okSafe argNames
	...
	{ "poker",         		"getaddressutxo",         &getaddressutxo,         true,  {"address"} },
