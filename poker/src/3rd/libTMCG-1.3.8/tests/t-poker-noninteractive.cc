	...
	(std::istream& vtmf_str, size_t player)
{
	...
	else
	{
		if (pid[player] == 0)
		{
            ...
            std::cout << "P_" << player << ".CheckGroup()" << std::endl;
            ...
            std::cout << "P_" << player << ".GenerateKey()" << std::endl;
			// create and exchange VTMF keys
			vtmf->KeyGenerationProtocol_GenerateKey();

	        std::cout << "P_" << player << ".h: " << vtmf->h << std::endl;
			...
			for (size_t type = 0; type < DECKSIZE; type++)
			{
                ...
                //std::cout << "c_" << type << ".c1: " << c.c_1 << std::endl << "c_" << type << ".c2: " << c.c_2 << std::endl;
			}
			...
			for (size_t i = 0; i < PLAYERS; i++)
			{
				...
				if (i == player)
				{
					...
					tmcg->TMCG_ProveStackEquality_Groth_noninteractive(s, s2,
						ss, vtmf, vsshe, lej);
					...
					*P_in[i] >> s2;
					...
					if (!tmcg->TMCG_VerifyStackEquality_Groth_noninteractive(s, s2,
						vtmf, vsshe, *P_in[i]))
					{
	...
	(int argc, char **argv)
{
	...
	std::cout << "p:" << vtmf->p << std::endl << "q:" << vtmf->q << std::endl << "g:" << vtmf->g <<std::endl << "k:" << vtmf->k << std::endl;
