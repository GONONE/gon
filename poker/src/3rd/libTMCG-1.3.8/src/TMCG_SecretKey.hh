...
struct TMCG_SecretKey
{
	...
	TMCG_SecretKey(const std::string& n, const std::string& e,
	 const unsigned long int keysize = TMCG_QRA_SIZE, 
	 const bool nizk_key = true);
	
	TMCG_SecretKey(const std::string& s);
	
	TMCG_SecretKey(const TMCG_SecretKey& that);
	
	TMCG_SecretKey& operator = (const TMCG_SecretKey& that);
	
	void generate(const unsigned long int keysize, const bool nizk_key);
	...
	std::string keyid(const size_t size = TMCG_KEYID_SIZE) const;
	
	size_t keyid_size(const std::string& s) const;
	
	std::string sigid(const std::string& s) const;
	
	bool import(std::string s);
	
	bool decrypt(unsigned char* value, std::string s) const;
	
	std::string sign(const std::string& data) const;
	
	std::string encrypt(const unsigned char* value) const;
	
	bool verify(const std::string& data, const std::string& s) const;
	
	~TMCG_SecretKey();
};

std::ostream& operator << (std::ostream& out, const TMCG_SecretKey& key);

std::istream& operator >> (std::istream& in, TMCG_SecretKey& key);
